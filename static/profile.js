...
if (cookiesAsJson.username && cookiesAsJson.password) {
    let owner = cookiesAsJson.username;
    let pass = cookiesAsJson.password;

    fetch(`/api/code/${owner}`, { body: JSON.stringify({ password: pass }), method: "POST", headers: { "Content-Type": "application/json" } })
        .then((data) => {
            data.json()
                .then((j_data) => {
                    if (j_data.code == 404) {
